...
targets="\
	...
	Rockchip|RK356X|aarch64|image \
	Rockchip|RK3576|aarch64|image \
	Rockchip|RK3588|aarch64|image \
...
do
	...
	# pick just the first UBOOT_SYSTEM board (if the device uses u-boot) to avoid
	# looping through every board variant during CI-style builds
	uboot_opt=""
	device_boards=$(./scripts/uboot_helper "${project}" "${device}" 2>/dev/null)
	if [ ${?} -eq 0 -a -n "${device_boards}" ]
	then
		first_board=$(echo ${device_boards} | cut -d' ' -f1)
		uboot_opt="UBOOT_SYSTEM=${first_board}"
		echo -en "\nUBOOT_SYSTEM is ${first_board} ...\n"
	else
		echo -en "\nNot UBOOT or Not find first_board ...\n"
	fi
	...
	then
		...
		make ${out} OFFICIAL=yes PROJECT=${project} DEVICE=${device} ARCH=${arch} MTIMMEDIATE=${qf} ${tc} ${uboot_opt} CUSTOM_LIBRETRO_CORES=none
		ret_nondb=${?}
		...
		make ${out} OFFICIAL=yes PROJECT=${project} DEVICE=${device} ARCH=${arch} MTIMMEDIATE=${qf} ${tc} ${uboot_opt} CUSTOM_LIBRETRO_CORES=none &>/dev/null &
		# store the pid
		pid=${!}
		finished=0
			...
			then
				...
				then
					...
					then
						echo ""
						echo "${statusline}"
						[ -n "${uboot_opt}" ] && echo -e "UBOOT_SYSTEM is ${first_board}" || true
